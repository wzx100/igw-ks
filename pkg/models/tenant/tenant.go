...
func defaultObjectFilter(object runtime.Object, filter query.Filter) bool {
	...
	case query.FieldNames:
		for _, name := range strings.Split(string(filter.Value), ",") {
			if workspace.ObjectMeta.Name == name {
				return true
			}
		}
		return false
	...
	case iamv1alpha2.FieldOptenantId:
		return workspace.Spec.OpTenantId == string(filter.Value)
	case query.FieldName:
		return strings.Contains(workspace.ObjectMeta.Name, string(filter.Value))
		// /namespaces?page=1&limit=10&uid=a8a8d6cf-f6a5-4fea-9c1b-e57610115706
	case query.FieldUID:
		return strings.Compare(string(workspace.ObjectMeta.UID), string(filter.Value)) == 0
		// /deployments?page=1&limit=10&namespace=kubesphere-system
	case query.FieldNamespace:
		return strings.Compare(workspace.ObjectMeta.Namespace, string(filter.Value)) == 0
		// /namespaces?page=1&limit=10&ownerReference=a8a8d6cf-f6a5-4fea-9c1b-e57610115706
	case query.FieldOwnerReference:
		for _, ownerReference := range workspace.ObjectMeta.OwnerReferences {
	...
	case query.FieldAnnotation:
		return labelMatch(workspace.ObjectMeta.Annotations, string(filter.Value))
		// /namespaces?page=1&limit=10&label=kubesphere.io/workspace:system-workspace
	case query.FieldLabel:
		return labelMatch(workspace.ObjectMeta.Labels, string(filter.Value))
	default:
		return false
	}
}
