...
import (
	...
	devopsv1alpha2 "kubesphere.io/kubesphere/pkg/kapis/devops"
...
func (s *APIServer) installKubeSphereAPIs(stopCh <-chan struct{}) {
	...
	tenantInterface := tenant.New(*optenantGetter, s.InformerFactory, s.KubernetesClient.Kubernetes(), s.KubernetesClient.KubeSphere(), s.EventsClient, s.LoggingClient, s.AuditingClient, amOperator, rbacAuthorizer, s.MonitoringClient, resourceGetter, stopCh)
	...
	urlruntime.Must(oauth.AddToContainer(s.container, imOperator,
		...
		auth.NewOAuthAuthenticator(amOperator, opTenantOperator, s.KubernetesClient.KubeSphere(), userLister, s.Config.AuthenticationOptions),
